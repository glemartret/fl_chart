...
|:---------------|:---------------|:-------|
|show| determines whether to show or hide the border	|true|
|border| [Border](https://api.flutter.dev/flutter/painting/Border-class.html) details that determine which border should be drawn with which color| Border.all(color: Colors.black, width: 1.0, style: BorderStyle.solid)|


### FlTitlesData
|PropName		|Description	|default value|
|:---------------|:---------------|:-------|
|show| determines whether to show or hide the titles around the chart|true|
...
|PropName		|Description	|default value|
|:---------------|:---------------|:-------|
|axisNameSize| Determines the size of the axis name | `16`|
|axisNameWidget| It shows the name of axis (you can pass a Widget)| `null`|
|sideTitles| It accepts a [SideTitles](#SideTitles) which is responsible for showing your axis side titles| `SideTitles()`|
...
|getTitlesWidget| A function to retrieve the title widget with a given value on the related axis.|defaultGetTitle|
...
|PropName|Description|default value|
|:-------|:----------|:------------|
|show|determines whether to show or hide the background grid data|true|
|drawHorizontalLine|determines whether to show or hide the horizontal grid lines|true|
|horizontalInterval|interval space of grid, leave it null to be calculated automatically |null|
|getDrawingHorizontalLine|a function to get the line style of each grid line by providing the related axis value|defaultGridLine|
|checkToShowHorizontalLine|a function to check whether to show or hide the horizontal grid by providing the related axis value |showAllGrids|
|drawVerticalLine|determines whether to show or hide the vertical grid lines|true|
|verticalInterval|interval space of grid, leave it null to be calculated automatically |null|
|getDrawingVerticalLine|a function to get the line style of each grid line by providing the related axis value|defaultGridLine|
|checkToShowVerticalLine|a function to determine whether to show or hide the vertical grid by providing the related axis value |showAllGrids|
...
|xError| Determines the error range of the data point using [FlErrorRange](#FlErrorRange) (which contains `lowerBy` and `upperValue`) for the x-axis|null|
|yError| Determines the error range of the data point using [FlErrorRange](#FlErrorRange) (which contains `lowerBy` and `upperValue`) for the y-axis|null|


### FlLine
|propName|Description|default value|
|:-------|:----------|:------------|
|color|determines the color of the line|Colors.black|
...
|strokeCap|strokeCap of the line, e.g. Setting to StrokeCap.round will draw the two ends of the line rounded. NOTE: this might not work on dash lines.|StrokeCap.butt|
...
|PropName|Description|default value|
|:-------|:----------|:------------|
|Picture|a Dart UI Picture which should be derived from the svg. See the example for how to get a Picture from an svg.|null|
...
|PropName| Description                                                                    | default value          |
|:-------|:-------------------------------------------------------------------------------|:-----------------------|
|lowerBy| Lower value of the error range. It is subtracted from the spot value and should be positive| null|
|upperBy| Upper value of the error range. It is added to the spot value and should be positive| null|
